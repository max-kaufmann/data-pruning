...
class Multiply(nn.Module):
    ...
    def __init__(self,mult_num):
        self.mult_num = mult_num
        super(Multiply,self).__init__()
    
    def forward(self,tensors):
        return tensors * self.mult_num
def get_model(weights,args):
    if weights == 'standard':
        model =  torch.hub.load('pytorch/vision:v0.10.0', 'resnet50', pretrained=True)
    elif weights == 'linf':
        model = ResNet(Bottleneck,[3, 4, 6, 3])#TODO: Taken from https://github.com/MadryLab, (it is the L_infinity epsilon=8/255 resnet 50 model) should cite if used in final paper.

        sd = torch.load(config.project_path + "/models/imagenet/data/weights/imagenet_linf_8.pt")['model']
        ...
        raise ValueError("Weights not recognised")
    
    model = torch.nn.Sequential(model,Multiply(args.mult_num))
    return model
