...
def shift_image(image, shift_vars):
    ...
    base_coords = torch.stack(torch.meshgrid(torch.arange(height),
                                             torch.arange(width),
                                             indexing="xy"),
                              axis=-1)
    base_coords = (
        (base_coords / torch.tensor([height, width], dtype=torch.float32)) -
        0.5) * 2
    base_coords = base_coords.to(config.device)

    shifted_coords_repeat = torch.repeat_interleave(shift_vars,
                                                    num_repeats_height,
                                                    axis=1)
    shifted_coords_repeat = torch.repeat_interleave(shifted_coords_repeat,
                                                    num_repeats_width,
                                                    axis=2)
...
class KlotskiAdversary(nn.Module):
    ...
    def forward(self, model, inputs, targets):
        ...
        if self.distance_metric == "l2":
            shift_vars = self.epsilon * self.normalise_tensor(
        ...
        if self.distance_metric == "linf":
            shift_vars = self.epsilon * \
                torch.rand(batch_size, self.num_blocks,
                           self.num_blocks, 2, device=config.device)
        ...
        for _ in range(0, self.num_steps):
            ...
            shift_vars = shift_vars + self.step_size * \
                self.normalise_tensor(grad.unsqueeze(1)).squeeze(1)
            shift_vars = self.project_tensor(shift_vars.unsqueeze(1),
                                             self.epsilon).squeeze(1)
...
class KlotskiAttack(AttackInstance):
    ...
    def __init__(self, model, args):
        ...
        self.attack = KlotskiAdversary(args.epsilon, args.num_steps,
                                       args.step_size, args.distance_metric,
                                       args.klotski_num_blocks)
        self.model = model
