...
def wood_noise(noise, image_size, num_rings,normalising_constant):
    ...
    xs, ys = torch.meshgrid(torch.arange(0, width),
                            torch.arange(0, height),
                            indexing="xy")
    xs, ys = xs.float().to(config.device), ys.float().to(config.device)
    xs, ys = xs.expand(batch_size, -1,
                       -1), ys.unsqueeze(0).expand(batch_size, -1, -1)
    ...
    dist = torch.sqrt(x_value * x_value + y_value * y_value) + \
        torch.nn.functional.interpolate(
            noise, (height, width), mode='bilinear').squeeze(1)
...
class WoodAdversary(nn.Module):
    ...
    def __init__(self, epsilon, num_steps, step_size, noise_resolution,
                 num_rings,random_init,normalising_constant):
        super().__init__()
    ...
    def forward(self, model, inputs, targets):
        ...
        if self.random_init:
            noise_variables = self.epsilon * \
                torch.rand((batch_size, 1, self.noise_resolution,
                        self.noise_resolution)).to(config.device)
        else:
            noise_variables = torch.zeros((batch_size, 1, self.noise_resolution,
                        self.noise_resolution)).to(config.device)
        ...
        for i in range(self.num_steps):
            ...
            noise = wood_noise(noise_variables, inputs.size(), self.num_rings,self.normalising_constant)
            ...
            noise_variables = noise_variables.clamp(-self.epsilon,
                                                    self.epsilon)
