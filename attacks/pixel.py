...
def pixelate_image(image, pixelated_image, pixel_variables):
    ...
    chunked_image = image.view(batch_size, num_channels, pixelated_height,
                               pixel_size, pixelated_width,
                               pixel_size).transpose(3, 4).contiguous()
    pixel_variables = pixel_variables.unsqueeze(-1).unsqueeze(-1)
    return_image = chunked_image * \
        (1 - pixel_variables) + \
        (pixelated_image.unsqueeze(-1).unsqueeze(-1)) * pixel_variables
    return_image = return_image.transpose(3, 4).contiguous().view(
...
class PixelAdversary(nn.Module):
    ...
    def __init__(self, epsilon, num_steps, step_size, pixel_size,
                 distance_metric):
        super().__init__()
    ...
    def forward(self, model, inputs, targets):
        ...
        even_pixelation = not (height % self.pixel_size == 0 and width % self.pixel_size == 0) 
        if even_pixelation:
            inputs = F.interpolate(inputs, size=(math.ceil(height / self.pixel_size)*self.pixel_size,math.ceil(width / self.pixel_size)*self.pixel_size), mode='bilinear', align_corners=False)

        pixelated_image = get_pixels(inputs, self.pixel_size)

        pixel_vars = self.epsilon * random.random() * self.normalise_tensor(
        ...
        for _ in range(0, self.num_steps):
            ...
            pixel_vars = pixel_vars + self.step_size * \
                self.normalise_tensor(grad)
...
class PixelAttack(AttackInstance):
    ...
    def __init__(self, model, args):
        ...
        self.attack = PixelAdversary(args.epsilon, args.num_steps,
                                     args.step_size, args.pixel_size,
