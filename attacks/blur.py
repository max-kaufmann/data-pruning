...
def dynamic_interpolation(image,
                          perturbed_image,
...
class BlurAdversary(nn.Module):
    ...
    def __init__(self, epsilon, num_steps, step_size, blur_kernel_size,
    ...
    def forward(self, model, inputs, targets):
        ...
        interp_matrix = (self.epsilon * torch.rand(
            (batch_size, 1, width, width), device=config.device)).detach()
        perturbed_inputs = blur_image(inputs, self.blur_kernel_size,
                                      self.blur_sigma).detach()
        interp_matrix.requires_grad = True

        #The inner loop
        for i in range(self.num_steps):

            adv_inputs = dynamic_interpolation(inputs, perturbed_inputs,
                                               interp_matrix,
