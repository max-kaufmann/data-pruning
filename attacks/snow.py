...
def make_snow_kernels(num_images,
                      num_layers,
                      snow_flake_length,
    ...
    for i in range(num_layers):
        ...
        rr, cc, val = weighted_line(
            mid, mid, np.random.randint(mid + 1, kernel_size),
            np.random.randint(mid + 1, kernel_size),
            np.random.choice([1, 3, 5], p=[0.6, 0.3, 0.1]), mid, kernel_size)

        k_npy[rr, cc] = val
        k_npy[:mid + 1, :mid + 1] = k_npy[::-1, ::-1][:(mid + 1), :(
            mid + 1
        ...
        kernel = torch.FloatTensor(k_npy.copy()).view(1, kernel_size,
                                                      kernel_size)

        gaussian_blur = torchvision.transforms.GaussianBlur(
            gaussian_kernel_size,
            gaussian_sigma)  # Add some blurring to make the line a bit thicker
...
def apply_snow_kernels(inputs, kernels):
    ...
    snow_grid = F.conv2d(inputs, kernels, padding=pad)

    return snow_grid.squeeze(1)


def get_mask(intensities, snow_grid_offsets, grid_separation):
    """
    Given a set of suitable kernels, and the variables which control the values in the sparse matrices,
    used to generate the snow, this generates several "layers of snow". Done by convolving the kernels with
    matrices whos entries are zero, apart from a grid of non-zero entries.

    intensities: the variables which control the values of non-zero grid entries
    kernels: the "snow kernels" which are passed over the grid
    snow_grid_offsets: controls the offset of the grids of snowflakes
    images_size: hold the size of the output image
    """

    # Get relevat size variables. Note the grabbing of the "num_slow_layers" variables, which
    # controls the number of overlaid snow grids.
    batch_size, num_snow_layers, intensity_grid_size, _ = intensities.size()

    # For convenience, we initalize a square matrix which perfectly fits a grid of the required size, and is
    # larger than the image we use.

    # We raise the entries to a power, to encourage spare matrices.
    # Initalize our matrices

    # This loop uses fancy indexing to set up the non-zero entries in our matrices, ensuring they are
    # controlled by the input "intensities" matrix.

    mask = torch.zeros(intensities.size())

    for i in range(0, num_snow_layers):
        offset_x = snow_grid_offsets[:, 0, i]
        offset_y = snow_grid_offsets[:, 1, i]
        a = np.arange(0, intensity_grid_size, grid_separation)
        xx, yy = np.meshgrid(a, a)
        xx = xx.reshape(1, xx.shape[0], xx.shape[1]) + \
            offset_x[:, np.newaxis, np.newaxis]
        yy = yy.reshape(1, yy.shape[0], yy.shape[1]) + \
            offset_y[:, np.newaxis, np.newaxis]
        mask[:, i][torch.arange(0, batch_size).view(-1, 1, 1), xx, yy] = 1
...
class SnowAdversary(nn.Module):
    ...
    def __init__(self, epsilon, distance_metric, num_steps, step_size,
                 flake_size, num_layers, grid_size, snow_init, image_discolour,
                 normalising_constant, gaussian_kernel_size, sigma_range):
        super().__init__()
        # Hyperparameters
        ...
        self.num_steps = num_steps  # The number of steps the optimisation algorithm takes
    ...
    def forward(self, model, inputs, targets):
        ...
        intensity_grid_size = math.ceil(
            image_size / self.grid_size) * self.grid_size

        # Generate the random offsets of each snowflake grid
        snow_grid_offsets = np.random.randint(0,
                                              self.grid_size,
                                              size=(batch_size, 2,
                                                    self.num_layers))
        ...
        flake_intensities = torch.exp(
            -1. / (self.snow_init) *
            (torch.rand(batch_size, self.num_layers, intensity_grid_size,
                        intensity_grid_size)))
        flake_intensities = self.project_tensor(flake_intensities,
                                                self.epsilon / 2)
        mask = get_mask(flake_intensities, snow_grid_offsets, self.grid_size)

        # create initial variables
        flake_intensities.requires_grad_()
        flake_intensities.to(config.device)

        kernels = make_snow_kernels(batch_size, self.num_layers,
                                    self.flake_size, self.kernel_size,
                                    self.sigma_range)

        # begin optimizing the inner loop
        for i in range(self.num_steps):
            t1 = time.time()
            snow_noise = make_snow(flake_intensities, mask, kernels,
                                   self.normalizing_constant)
            adv_inputs = apply_snow(inputs.detach(), snow_noise,
                                    self.image_discolour)
        ...
        snow_noise = make_snow(flake_intensities, mask, kernels,
                               self.normalizing_constant)
        adv_inputs = apply_snow(inputs.detach(), snow_noise,
                                self.image_discolour).clamp(0, 1)
...
class SnowAttack(AttackInstance):
    ...
    def __init__(self, model, args):
        ...
        self.attack = SnowAdversary(
            args.epsilon, args.distance_metric, args.num_steps, args.step_size,
            args.snow_flake_size, args.snow_num_layers, args.snow_grid_size,
            args.snow_init, args.snow_image_discolour,
            args.snow_normalizing_constant, args.snow_kernel_size,
            (args.snow_sigma_range_lower, args.snow_sigma_range_upper))
        self.model = model
