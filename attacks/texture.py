...
def get_thin_kernels(start=0, end=360, step=45):
    ...
    for angle in range(start, end, step):
        ...
        # apply rotation
        kernel_angle_increased = torchvision.transforms.functional.rotate(torch.from_numpy(thin_kernel_0).unsqueeze(0),angle,torchvision.transforms.InterpolationMode.BILINEAR).squeeze(0).numpy()
        # get the k=3 kerne
    ...
    def forward(self, img, threshold):
        with torch.no_grad():
            ...
            thin_edges = thin_edges / \
                (torch.max(thin_edges.reshape(B, -1), dim=1)
                 [0].reshape(B, 1, 1, 1))
...
class TextureAdversary(nn.Module):
    ...
    def forward(self, model, inputs, targets):
        ...
        if self.distance_metric == "l2":
            interp_vars = self.epsilon * self.normalise_tensor(
                torch.rand(batch_size, height, width,
        ...
        for i in range(self.num_steps):
            ...
            adv_inputs = dynamic_interpolation(inputs, edge_image, interp_vars,
                                               edges)
...
class TextureAttack(AttackInstance):
    ...
    def __init__(self, model, args):
        ...
        self.attack = TextureAdversary(epsilon=args.epsilon,
