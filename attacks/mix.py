...
def dynamic_interpolation(image, perturbed_image, interpolation_matrix,
                          kernel_size, sigma):
    ...
    adv_img = interpolation_matrix*perturbed_image + \
        (1-interpolation_matrix)*image
...
def get_mix_images(images, targets):
    ...
    This is an O(n^2) operation, but as long as your batch size is < 100,000 it should be fine :)."""
...
class MixAdversary(nn.Module):
    ...
    def forward(self, model, inputs, targets):
        a_start = time.time()
        inputs, targets = inputs.to(config.device).detach(), targets.to(
            config.device).detach()
        """
        ...
        interp_matrix = (self.epsilon * self.normalise_tensor(
            torch.rand((batch_size, 1, height, width),
        ...
        for i in range(self.num_steps):
            ...
            adv_inputs = dynamic_interpolation(inputs, mix_images,
                                               interp_matrix,
            ...
            grad = torch.autograd.grad(loss, interp_matrix,
                                       only_inputs=True)[0]
        ...
        adv_inputs = dynamic_interpolation(inputs, mix_images, interp_matrix,
                                           self.interp_kernel_size,
                                           self.interp_kernel_sigma)
        a_end = time.time()
...
class MixAttack(AttackInstance):
    ...
    def __init__(self, model, args):
        ...
        self.attack = MixAdversary(args.epsilon, args.num_steps,
                                   args.step_size, args.mix_interp_kernel_size,
                                   args.mix_interp_kernel_sigma,args.distance_metric)
