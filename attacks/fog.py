...
def fog_creator(fog_vars, bsize, mapsize, wibbledecay):
    ...
    maparray = torch.from_numpy(
        np.zeros((bsize, mapsize, mapsize), dtype=np.float32)).to(
            config.device
        )  
    maparray[:, 0, 0] = 0
    stepsize = mapsize 
    ...
    def wibbledmean(array, var_num):
        ...
        result = array / 4. + wibble * fog_vars[var_num]
        return result
    ...
    def fillsquares(var_num):
        ...
        maparray[:, stepsize // 2:mapsize:stepsize,
                 stepsize // 2:mapsize:stepsize] = wibbledmean(
                     squareaccum, var_num)
        return var_num + 1
    ...
    def filldiamonds(var_num):
        ...
        maparray[:, 0:mapsize:stepsize,
                 stepsize // 2:mapsize:stepsize] = wibbledmean(ltsum, var_num)
        var_num += 1
        tdrsum = drgrid + torch.roll(drgrid, 1, 2)
        tulsum = ulgrid + torch.roll(ulgrid, -1, 1)
        ttsum = tdrsum + tulsum
        maparray[:, stepsize // 2:mapsize:stepsize,
                 0:mapsize:stepsize] = wibbledmean(ttsum, var_num)
        return var_num + 1
...
def apply_fog(inputs,fog,grey=torch.tensor([0.485,0.45,0.406],device=config.device)):
...
class FogAdversary(nn.Module):
    ...
    def __init__(self, epsilon, num_steps, step_size, wibbledecay,
                 distance_metric):
        super().__init__()
    ...
    def forward(self, model, inputs, targets):
        ...
        for i in range(bits):
            for j in range(3):
                fog_vars.append(
                    self.epsilon*torch.rand((bsize, 2**i, 2**i),
                               requires_grad=True,
        ...
        for i in range(self.num_steps):
            ...
            fog = fog_creator(fog_vars, bsize, nearest_power_of_2,
                              self.wibbledecay)
            if padding != 0:
...
class FogAttack(AttackInstance):
    ...
    def __init__(self, model, args):
        ...
        self.attack = FogAdversary(args.epsilon, args.num_steps,
                                   args.step_size, args.fog_wibbledecay,
