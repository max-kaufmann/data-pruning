...
def glitch_image(images, glitch_vars, num_lines):
    ...
    displacement_tensor = torch.repeat_interleave(glitch_vars.unsqueeze(-1), line_width, dim=2) 
    ...
    xx, yy = torch.meshgrid(torch.arange(0, width),
                            ...
                            indexing='xy')
    xx, yy = (xx.repeat(batch_size, num_channels, 1, 1)/width)*2 - \
        1, (yy.repeat(batch_size, num_channels, 1, 1)/height)*2 - 1
    ...
    for j in range(0, num_channels):
        image_channels.append(F.grid_sample(images[:, j:j + 1],
                                                  new_location[:, j],
...
def grey_images(images,
                grey_strength,
                grey=torch.tensor((0.87, 0.87, 0.87)).to(config.device)):
    """
    Greys out the image by adding a constant to each pixel
    """

    return_images = images*(1-grey_strength) + \
        grey.unsqueeze(-1).unsqueeze(-1)*grey_strength
    return return_images
...
class GlitchAdversary(nn.Module):
    ...
    def __init__(self, epsilon, num_steps, step_size, num_lines,
                 grey_strength):
    ...
    def forward(self, model, inputs, targets):
        ...
        glitch_vars = self.epsilon * \
            torch.rand(batch_size, num_channels,
                       self.num_lines, requires_grad=True)

        if height % self.num_lines != 0:
            raise ValueError(
                "The number of lines must divide the height of the image")
...
class GlitchAttack(AttackInstance):
    ...
    def __init__(self, model, args):
        ...
        self.attack = GlitchAdversary(epsilon=args.epsilon,
