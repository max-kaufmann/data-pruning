...
def image_to_patches(image):
    ...
    image_reshaped = image.view(batch_size, num_channels, height // k, k,
                                width // k, k)
...
def patches_to_image(patches, image_shape):
    ...
    image_reshaped = patches.view(batch_size, num_channels, height // k,
                                  width // k, k, k)
    image_transposed = torch.transpose(image_reshaped, 3, 4)
...
def ycbcr_to_rgb_jpeg(image):
    ...
    matrix_yuv = torch.tensor(np.array(
...
def rgb_to_ycbcr_jpeg(image):
    ...
    matrix_rgb = torch.as_tensor(np.array(
        [[0.299, 0.587, 0.114], [-0.168736, -0.331264, 0.5],
         [0.5, -0.418688, -0.081312]],
        dtype=np.float32).T,
                                 device=config.device)

    shift_rgb = torch.as_tensor([0., 128., 128.], device=config.device)
...
def jpeg_encode(image, noise_vars):
    ...
    comp = comp + noise_vars  # Add the noise variables to the frequency domain coefficients
...
def jpeg_decode(components, image_shape):
    ...
    image = torch.clamp(image / 255., 0, 1)
...
class JPEGAdversary(nn.Module):
    ...
    def forward(self, model, inputs, targets):
        ...
        noise_vars = self.epsilon * torch.rand((batch_size, num_channels, height // 8 * width//8, 8, 8)).to(config.device)
...
class JPEGAttack(AttackInstance):
    ...
    def __init__(self, model, args):
        ...
        self.attack = JPEGAdversary(args.num_steps, args.step_size,
                                    args.epsilon)
