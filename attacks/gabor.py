...
def get_gabor(k_size, sigma, Lambda, theta):
    ...
    g = torch.exp(-0.5 * (rotx**2 / (sigma + 1e-3)**2 + roty**2 /
                          (sigma + 1e-3)**2))
...
def get_gabor_with_sides(k_size, sigma, Lambda, theta, sides=3):
    """Creates "sides" for the gabor kerne"""
    g = get_gabor(k_size, sigma, Lambda, theta)
    for i in range(1, sides):
        g += get_gabor(k_size, sigma, Lambda,
                       theta + np.pi * i / sides) / sides
    return g
...
def create_gabor_kernel(batch_size, k_size, sigma, sides):
    ...
    Lambda = ((k_size / 4. - 3) * torch.rand(1) + 3).to(config.device)

    for b in range(batch_size):
        kernels.append(
            get_gabor_with_sides(k_size, sigma, Lambda, theta, sides))
...
def generate_random_distribution(g_vars, mask, image, batch_size, k_size,
                                 sigma, sides, size):
    ...
    for g_conv in g_vars: # Due to weirdness with grouped convolution, the for loop is more efficient than using a single grouped convolution 
        batch_size = g_conv.size(0)
        g_conv = g_conv.view(1, batch_size, g_conv.size(-2), g_conv.size(-1))
        g_conv = F.conv2d(g_conv,
                          weight=gabor_kernel,
                          stride=1,
                          groups=batch_size,
...
class GaborAdversary(nn.Module):
    ...
    def forward(self, model, inputs, targets):
        ...
        # Initalise the base
        sp_conv_numpy = sparse.random(num_image_channels * img_size *
                                      batch_size,
                                      img_size,
                                      density=self.weight_density,
                                      format='csr')
        gabor_vars = torch.FloatTensor(sp_conv_numpy.todense()).view(
            num_image_channels, batch_size, img_size,
            img_size).to(config.device)
        ...
        for i in range(self.num_steps):
            ...
            gabor_noise = generate_random_distribution(gabor_vars, mask,
                                                       inputs, batch_size,
                                                       self.kernel_size,
                                                       self.sigma, self.sides,
                                                       img_size)
        ...
        gabor_noise = generate_random_distribution(gabor_vars, mask, inputs,
                                                   batch_size,
                                                   self.kernel_size,
                                                   self.sigma, self.sides,
                                                   img_size)
...
class GaborAttack(AttackInstance):
    ...
    def __init__(self, model, args):
        ...
        self.attack = GaborAdversary(args.num_steps, args.step_size,
                                     args.epsilon, args.gabor_kernel_size,
                                     args.gabor_sides, args.gabor_sigma,
                                     args.gabor_weight_density,
