...
def rgb2hsv(rgb):
    ...
    cmax_idx[delta == 0] = 3
    hsv_h[cmax_idx == 0] = (((rgb[:, 1:2] - rgb[:, 2:3]) / delta) %
                            6)[cmax_idx == 0]
    hsv_h[cmax_idx == 1] = (((rgb[:, 2:3] - rgb[:, 0:1]) / delta) +
                            2)[cmax_idx == 1]
    hsv_h[cmax_idx == 2] = (((rgb[:, 0:1] - rgb[:, 1:2]) / delta) +
                            4)[cmax_idx == 2]
    hsv_h[cmax_idx == 3] = 0.
    hsv_h /= 6.
...
class HSVAdversary(nn.Module):
    ...
    def __init__(self, epsilon, num_steps, step_size, distance_metric,
        ...
        if distance_metric == "linf":
            self.clamp_tensor = lambda x, epsilon: torch.clamp(
                x, -epsilon, epsilon)
            ...
            raise ValueError(
                f"The distance metric {distance_metric} is not supported")
    ...
    def forward(self, model, inputs, targets):
        ...
        hsv_perturb = self.epsilon * \
            torch.rand(inputs.shape, device=config.device).to(config.device)
        hsv_perturb.requires_grad = True

        for i in range(self.num_steps):

            adv_inputs = add_hsv_perturbation(hsv_perturb, inputs.detach(),
                                              self.epsilon, self.kernel_size,
                                              self.sigma)
...
class HsvAttack(AttackInstance):
    ...
    def __init__(self, model, args):
        ...
        self.attack = HSVAdversary(args.epsilon, args.num_steps,
                                   args.step_size, args.distance_metric,
                                   args.hsv_kernel_size, args.hsv_sigma)
