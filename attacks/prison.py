...
def get_bar_variables(image, num_bars, bar_width, grey):
    ...
    bar_locations = torch.arange(
        ...
        width - math.floor(width / num_bars) // 2, width // num_bars)
    ...
    for b in bar_locations:
        mask += torch.logical_and(((b - bar_width // 2) <= xx),
                                  (xx <=
                                   (b + bar_width // 2))).to(config.device)

    bar_variables = torch.zeros(image.size(),
...
class PrisonAdversary(nn.Module):
    ...
    def forward(self, model, inputs, targets):
        ...
        inputs, targets = inputs.clone().to(config.device), targets.to(
            config.device)
        
        # We have a mask to control what is optimised, and varialbes which control the colour of the bars
        bar_variables, mask = get_bar_variables(inputs, self.num_bars,
                                                self.bar_width, self.grey)
        ...
        for i in range(self.num_steps):
            ...
            grad = torch.autograd.grad(loss, bar_variables,
                                       only_inputs=True)[0]
...
class PrisonAttack(AttackInstance):
    ...
    def __init__(self, model, args):
        ...
        self.attack = PrisonAdversary(epsilon=args.epsilon,
