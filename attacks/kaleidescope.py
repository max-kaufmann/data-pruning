...
def get_random_circle(image_size, radius):
    ...
    xx, yy = torch.meshgrid(torch.arange(width).to(config.device),
...
def get_random_equilateral_triangle(image_size, edge_size):
    ...
    batch_size, num_channels, height, width = image_size
    triangle_mesh = torch.zeros((height, width),
                                dtype=bool).to(config.device)


    x_1, y_1 = random.randrange(edge_size + 1,
                                width - edge_size - 1), random.randrange(
                                    edge_size + 1, height - edge_size - 1)

    theta_1 = random.random() * 2 * math.pi
    x_2, y_2 = int(x_1 + math.cos(theta_1) *
                    edge_size), int(y_1 + math.sin(theta_1) * edge_size)

    theta_2 = theta_1 + math.pi / 3
    x_3, y_3 = int(x_1 + math.cos(theta_2) *
                    edge_size), int(y_1 + math.sin(theta_2) * edge_size)
...
def fill_triangle(p1, p2, p3, fill_tensor):
    ...
    if p2[1] == p3[1]:
        ...
        if p1[1] == p2[1]:
            ...
            newx = int(p1[0] + (float(p2[1] - p1[1]) / float(p3[1] - p1[1])) *
                       (p3[0] - p1[0]))
            newy = p2[1]
            p_new = (newx, newy)
...
def fill_flat_triangle(p1, p2, p3, tensor):
    ...
    for curr_h in height_range:
        ...
        xs = torch.arange(max(0, int(x_1)), max(min(int(x_2), input_height),
                                                0))
...
def get_random_square(image_size, edge_size):
    ...
    batch_size, num_channels, height, width = image_size
    square_mask = torch.zeros(( height, width),
                              dtype=bool).to(config.device)


    x_1, y_1 = random.randrange(
        edge_size + 1, int(width - edge_size - 1)), random.randrange(
            edge_size  +1, int(height - edge_size - 1))
    theta_1 = random.random() * math.pi * 2
    theta_2 = theta_1 + math.pi / 2

    dx_1, dy_1 = edge_size * math.cos(theta_1), edge_size * math.sin(
        theta_1)
    dx_2, dy_2 = edge_size * math.cos(theta_2), edge_size * math.sin(
        theta_2)
...
def init_colours(batch_size,num_shapes, min_value_valence,
                 min_value_saturation):
    colours = torch.zeros((batch_size, 3, num_shapes, 1)).to(config.device)

    colours[:, 0:1, :, 0:1] = torch.rand(
        (batch_size, 1, num_shapes, 1)).to(config.device)
    colours[:, 1:2, :, 0:1] = torch.rand((batch_size, 1, num_shapes, 1)).to(
        config.device) * (1 - min_value_saturation) + min_value_saturation
    colours[:, 2:3, :, 0:1] = torch.rand((batch_size, 1, num_shapes, 1)).to(
        config.device) * (1 - min_value_valence) + min_value_valence
...
def add_shapes(image_batch, shape_masks,edge_mask, colour, transparency):
    ...
    shape_masks_extended = shape_masks.unsqueeze(1).unsqueeze(0)
    # broadcasting to give each shape a colou
    shapes_colour = shape_masks_extended.float() * colour.unsqueeze(
        -1).unsqueeze(-1)
    shapes_colour = shapes_colour.sum(dim=1)
    shape_masks_combined = shape_masks_extended.sum(dim=1) > 0
    image_uncoloured = (1 - shape_masks_combined.float() *
                        (1 - transparency)) * image_batch

    image_batch = image_uncoloured + shapes_colour


    image_batch[...,edge_mask]=0
    return image_batch
...
def get_edge_masks(shape_mask,edge_width):
    ...
    bigger_shape = F.conv2d(shape_mask.unsqueeze(0).float(),kernels,padding=edge_width//2,groups=num_shapes).squeeze(0)
    
    bigger_shape_mask = bigger_shape > 0
    edge_mask_seperate = torch.logical_and(bigger_shape_mask,torch.logical_not(shape_mask.squeeze(0).squeeze(0) > 0))

    edge_mask = edge_mask_seperate.sum(dim=0) > 0
    return edge_mask 

def add_variables(image_batch, shape_masks, colour_variables,edge_mask,edge_variables):
    shape_masks_extended = shape_masks.unsqueeze(0).unsqueeze(2)
    # broadcasting to give each shape a colou
    shapes_colour = shape_masks_extended.float() * colour_variables.unsqueeze(
        -1).unsqueeze(-1)
    shapes_colour = shapes_colour.sum(dim=1)

    image_batch = image_batch + shapes_colour

    edge_colour = torch.abs(edge_variables).repeat(1,3,1,1) * edge_mask.unsqueeze(0).unsqueeze(0).float()
    image_batch = image_batch + edge_colour

    return image_batch.clamp(0, 1)


class KaleidescopeAdversary(nn.Module):

    def __init__(self, epsilon, num_steps, step_size, num_shapes, shape_size,
                 min_value_valence, min_value_saturation, transparency,edge_width):
        """
        Implements the Kaleidescope Adversary, whcih involves adding random shapes to an image and then optimising their colour.
    ...
    def forward(self, model, inputs, targets):
        ...
        colour_init = init_colours(batch_size,self.num_shapes,
                                   self.min_value_valence,
                                   self.min_value_saturation)
        colour_vars = (torch.rand(size=colour_init.shape).to(config.device) -
                       0.5) * self.epsilon * 2
        colour_vars.requires_grad = True
        edge_vars = torch.rand(size=(batch_size,1,height,width)).to(config.device) * self.epsilon 
        edge_vars.requires_grad = True

        shape_mask = get_shape_masks(inputs.size(), self.num_shapes,
                                     self.shape_size)
        edge_mask = get_edge_masks(shape_mask, self.edge_width)

        inputs = add_shapes(inputs, shape_mask,edge_mask, colour_init, self.transparency)


        # The inner loop
        for _ in range(self.num_steps):

            adv_inputs = add_variables(inputs, shape_mask, colour_vars,edge_mask,edge_vars)

            outputs = model(adv_inputs)
            loss = F.cross_entropy(outputs, targets)

            colour_grads,edge_grads = torch.autograd.grad(loss, [colour_vars,edge_vars])
...
class KaleidescopeAttack(AttackInstance):

    def __init__(self, model, args):
        super().__init__(model, args)
        self.attack = KaleidescopeAdversary(
            epsilon=args.epsilon,
            num_steps=args.num_steps,
            step_size=args.step_size,
            num_shapes=args.kaleidescope_num_shapes,
            shape_size=args.kaleidescope_shape_size,
            min_value_valence=args.kaleidescope_min_value_valence,
            min_value_saturation=args.kaleidescope_min_value_saturation,
            transparency=args.kaleidescope_transparency,edge_width=args.kaleidescope_edge_width)
...
def get_attack(model, args):
    return KaleidescopeAttack(model, args)
