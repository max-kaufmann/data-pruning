...
def get_coordinates(image):
    ...
    xx, yy = torch.meshgrid(torch.arange(0, width),
                            torch.arange(0, height),
                            indexing="xy")
    # We then normalise the coridnates between -1 and 1, as this is the range of the grid_sample function
    coords = torch.stack([((xx / width) - 0.5) * 2, ((yy / height) - 0.5) * 2],
                         dim=-1).repeat(batch_size, 1, 1, 1)
...
def swirl_image(images, centers, rotation_strengths, radii, min_strengths):
    ...
    swirl_locations = swirl_mapping(coordinates, centers, rotation_strengths,
                                    radii, min_strengths)

    return torch.nn.functional.grid_sample(images,
                                           swirl_locations,
                                           mode='bilinear',
                                           padding_mode='zeros',
                                           align_corners=None)
...
class WhirlpoolAdversary(nn.Module):
    ...
    def __init__(self, epsilon, num_steps, step_size, distance_metric,
                 num_whirlpools, whirlpool_radius, whirlpool_min_strength):
        super().__init__()
    ...
    def forward(self, model, inputs, targets):
        ...
        if self.distance_metric == "l2":
            whirlpool_strength_vars = self.epsilon * self.normalise_tensor(
                torch.rand(batch_size,
                           self.num_whirlpools,
                           device=config.device)) * random.random()
        if self.distance_metric == "linf":
            whirlpool_strength_vars = self.epsilon * \
                torch.rand(batch_size, self.num_whirlpools,
                           device=config.device)

        whirlpool_strength_vars.requires_grad = True

        whirlpool_centers = (torch.rand(
            batch_size, self.num_whirlpools, 2, device=config.device) -
                             0.5) * 2

        for _ in range(0, self.num_steps):
            adv_inputs = swirl_image(inputs, whirlpool_centers,
            ...
            whirlpool_strength_vars = whirlpool_strength_vars + self.step_size * \
                self.normalise_tensor(grad.unsqueeze(1)).squeeze(1)
            whirlpool_strength_vars = self.project_tensor(
                whirlpool_strength_vars.unsqueeze(1), self.epsilon).squeeze(1)
...
class WhirlpoolAttack(AttackInstance):
    ...
    def __init__(self, model, args):
        ...
        self.attack = WhirlpoolAdversary(args.epsilon, args.num_steps,
                                         args.step_size, args.distance_metric,
                                         args.num_whirlpools,
                                         args.whirlpool_radius,
                                         args.whirlpool_min_strength)
        self.model = model
